...
## Bonus: Exposing to MCP

If you pair with AI coding agents (Claude Code, Cursor, Codex), there’s a neat zero-effort side effect: adding a `# @desc` comment above any function automatically registers it as a typed tool over [MCP](https://modelcontextprotocol.io) when running `run --serve-mcp`:
...
One comment line gives you clean shell help text for humans *and* hands your coding agent a deterministic tool it can discover and run without guessing flags or hallucinating syntax.
